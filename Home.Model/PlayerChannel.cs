...
{
    private static Props Props = Props.Create(() => new PlayerActor());
    ...
    public PlayerChannel(ITcpSocketServer server, IChannel channel,
        ...
        serverEvent)
    {
        logger = new NLogAdapter(ConnectionId);
    }
    ...
    public async void BindPlayerActor(Request message)
    {
        ...
        A.RequireNotNull(actor, Code.Error, "player actor not found, login api may be overdue， please relogin",
            true);
        //填充链接id
...
internal static class PlayerChannelService
{
}