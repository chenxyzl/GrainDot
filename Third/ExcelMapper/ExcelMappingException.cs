...
public class ExcelMappingException : Exception
{
    ...
    public ExcelMappingException(string message, ExcelSheet sheet, int rowIndex, int columnIndex) : base(
        GetMessage(message, sheet, rowIndex, columnIndex))
    {
    }
    ...
    /// <param name="rowIndex">The zero-based index of the row in the sheet that is currently being read.</param>
    /// <param name="innerException">The inner exception of the exception.</param>
    public ExcelMappingException(string message, ExcelSheet sheet, int rowIndex, int columnIndex,
        Exception innerException) : base(GetMessage(message, sheet, rowIndex, columnIndex), innerException)
    {
    }
    ...
    private static string GetMessage(string message, ExcelSheet sheet, int rowIndex, int columnIndex)
    {
        ...
        if (columnIndex != -1)
        {
            ...
            {
                if (sheet.Heading == null) sheet.ReadHeading();
