...
public class ManyToOneEnumerableMap<TElement> : IMap
{
    ...
    /// </summary>
    public ManyToOneEnumerableMap(IMultipleCellValuesReader cellValuesReader,
        IValuePipeline<TElement> elementPipeline, CreateElementsFactory<TElement> createElementsFactory)
    {
        ...
        CreateElementsFactory =
            createElementsFactory ?? throw new ArgumentNullException(nameof(createElementsFactory));
    }
        ...
        out object value)
    {
        ...
        if (excelColumnNameAttribute == null)
            throw new ExcelMappingException(
                $"Could not find ExcelColumnNameAttribute for \"{excelColumnNameAttribute.Name}\"", sheet, rowIndex,
                -1);
        ...
        for (var idx = 1; idx <= excelColumnMultiAttribute.Count; idx++)
        {
            ...
            if (columnNamesDic.ContainsKey(keyName)) keyIndex.Add(results.ToArray()[columnNamesDic[keyName]]);
        }

        var elements = new List<TElement>();
        foreach (var result in keyIndex)
        {
            var elementValue =
                (TElement) ValuePipeline.GetPropertyValue(ElementPipeline, sheet, rowIndex, reader, result, member);
    ...
    ///     or field.
    /// </summary>
    /// <param name="elementMap">
    ///     The pipeline that maps the value of a single cell to an object of the element type of the property
    ///     or field.
    /// </param>
    /// <returns>The property map that invoked this method.</returns>
    public ManyToOneEnumerableMap<TElement> WithElementMap(
        Func<IValuePipeline<TElement>, IValuePipeline<TElement>> elementMap)
    {
