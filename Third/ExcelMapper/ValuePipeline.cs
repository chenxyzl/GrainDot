...
namespace ExcelMapper
{
    ...
    public class ValuePipeline : IValuePipeline
    {
        ...
        internal static object GetPropertyValue(IValuePipeline pipeline, ExcelSheet sheet, int rowIndex, IExcelDataReader reader, ReadCellValueResult readResult, MemberInfo member)
        {
            ...
            {
                readResult = new ReadCellValueResult(readResult.ColumnIndex, readResult.ColumnName, transformer.TransformStringValue(sheet, rowIndex, readResult));
            }

            //member.GetCustomAttribute<ExcelColumnNameAttribute>().AllowNull
            if (string.IsNullOrEmpty(readResult.StringValue))
            {
                if (member.GetCustomAttribute<ExcelColumnNameAttribute>().AllowNull || member.GetCustomAttribute<ExcelColumnNameAttribute>().Default != null)
                {
                    return member.GetCustomAttribute<ExcelColumnNameAttribute>().Default;
                }
                else if (pipeline.EmptyFallback != null)
                {
