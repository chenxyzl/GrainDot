...
internal static class ImmutableCollectionUtilities
{
    ...
    public static bool IsImmutableEnumerableType(this Type type)
    {
        if (!type.IsGenericType ||
            !type.Assembly.FullName.StartsWith("System.Collections.Immutable,", StringComparison.Ordinal))
            return false;
    ...
    private static Type GetImmutableEnumerableConstructingType(Type type)
    {
        ...
        var underlyingType = type.GetGenericTypeDefinition();
        ...
        switch (underlyingType.FullName)
        {
            ...
            default:
                Debug.Assert(
                    underlyingType.FullName == ImmutableHashSetGenericTypeName ||
                    underlyingType.FullName == ImmutableSetGenericInterfaceTypeName,
                    $"Unknown type {underlyingType.FullName}");
    ...
    private static Type GetImmutableDictionaryConstructingType(Type type)
    {
        ...
        switch (underlyingType.FullName)
        {
            ...
            case ImmutableDictionaryGenericInterfaceTypeName:
                ...
                break;
            default:
                Debug.Assert(underlyingType.FullName == ImmutableSortedDictionaryGenericTypeName,
                    $"Unknown type {underlyingType.FullName}");
    ...
    {
        var constructingType = GetImmutableEnumerableConstructingType(type);
    ...
    {
        var constructingType = GetImmutableDictionaryConstructingType(type);
        return constructingType.GetMethods()
            .First(m => m.Name == "CreateRange" && m.GetParameters().Length == 1)
