...
public static class ExcelImporterUtils
{
    ...
    public static IEnumerable<ExcelClassMap> RegisterClassMapsInNamespace(this ExcelImporter importer,
        Assembly assembly, string namespaceString)
    {
        ...
        var classMaps =
            classMapTypes.Select(Activator.CreateInstance).OfType<ExcelClassMap>().ToArray();
        if (classMaps.Length == 0)
            throw new ArgumentException(
                $"No classmaps found in the namespace \"{namespaceString}\" in the assembly \"{assembly}\".",
                nameof(namespaceString));

        foreach (var classMap in classMaps) importer.Configuration.RegisterClassMap(classMap);
