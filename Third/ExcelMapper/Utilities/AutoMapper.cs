...
namespace ExcelMapper.Utilities
{
    public static class AutoMapper
    {
        ...
        internal static bool TryCreateGenericDictionaryMap<TKey, TValue>(Type memberType, FallbackStrategy emptyValueStrategy, out ManyToOneDictionaryMap<TKey, TValue> map)
        {
            ...
            if (!TryCreatePrimitivePipeline<TKey>(emptyValueStrategy, out ValuePipeline<TKey> keyPipeline))
            {
                map = null;
                return false;
            }

            if (!TryGetCreateDictionaryFactory<TKey, TValue>(memberType, out CreateDictionaryFactory<TKey, TValue> factory))
            {
                map = null;
                return false;
            }
        ...
        private static bool TryGetCreateDictionaryFactory<TKey, TValue>(Type memberType, out CreateDictionaryFactory<TKey, TValue> result)
        {
            ...
            if (memberType.GetTypeInfo().IsInterface)
            {
                if (memberType.GetTypeInfo().IsAssignableFrom(typeof(Dictionary<TKey, TValue>).GetTypeInfo()))
                {
                    result = elements =>
                    {
                        ...
                        foreach (KeyValuePair<TKey, TValue> keyValuePair in elements)
                        {
            ...
            else if (memberType.ImplementsInterface(typeof(IDictionary<TKey, TValue>)))
            {
                result = elements =>
                {
                    IDictionary<TKey, TValue> dictionary = (IDictionary<TKey, TValue>)Activator.CreateInstance(memberType);
                    foreach (KeyValuePair<TKey, TValue> keyValuePair in elements)
                    {
