...
public static class AutoMapper
{
    ...
    private static MethodInfo TryCreateMemberMapMethod => s_tryCreateMemberMapMethod ??
                                                          (s_tryCreateMemberMapMethod = typeof(AutoMapper)
                                                              .GetTypeInfo()
                                                              .GetDeclaredMethod(nameof(TryCreateMemberMap)));

    private static MethodInfo TryCreateGenericEnumerableMapMethod => s_tryCreateGenericEnumerableMapMethod ??
                                                                     (s_tryCreateGenericEnumerableMapMethod =
                                                                         typeof(AutoMapper).GetTypeInfo()
                                                                             .GetDeclaredMethod(
                                                                                 nameof(
                                                                                     TryCreateGenericEnumerableMap)));

    private static MethodInfo TryCreateGenericDictionaryMapMethod => s_tryCreateGenericDictionaryMapMethod ??
                                                                     (s_tryCreateGenericDictionaryMapMethod =
                                                                         typeof(AutoMapper).GetTypeInfo()
                                                                             .GetDeclaredMethod(
                                                                                 nameof(
                                                                                     TryCreateGenericDictionaryMap)));
    ...
    private static bool TryCreateMemberMap<T>(MemberInfo member, FallbackStrategy emptyValueStrategy, out IMap map)
    {
        ...
        if (TryCreateDictionaryMap<T>(emptyValueStrategy, out var dictionaryMap))
        {
            map = dictionaryMap;
            return true;
        }
        ...
        if (TryCreateEnumerableMap(member, emptyValueStrategy, out var multiMap))
        {
            map = multiMap;
            return true;
        }
    ...
    internal static bool TryCreatePrimitivePipeline<T>(FallbackStrategy emptyValueStrategy,
        out ValuePipeline<T> pipeline)
    {
        if (!TryGetWellKnownMap(typeof(T), emptyValueStrategy, out var mapper,
                out var emptyFallback, out var invalidFallback))
        {
            pipeline = null;
            return false;
        }
    ...
    internal static bool TryCreatePrimitiveMap<T>(MemberInfo member, FallbackStrategy emptyValueStrategy,
        out OneToOneMap<T> map)
    {
        if (!TryGetWellKnownMap(typeof(T), emptyValueStrategy, out var mapper,
                out var emptyFallback, out var invalidFallback))
        {
        ...
        var defaultReader = GetDefaultSingleCellValueReader(member);
    ...
    internal static ISingleCellValueReader GetDefaultSingleCellValueReader(MemberInfo member)
    {
        ...
        if (colummnNameAttribute != null) return new ColumnNameValueReader(colummnNameAttribute.Name);

        var colummnIndexAttribute = member.GetCustomAttribute<ExcelColumnIndexAttribute>();
        if (colummnIndexAttribute != null) return new ColumnIndexValueReader(colummnIndexAttribute.Index);

        return new ColumnNameValueReader(member.Name);
    }

    private static bool TryGetWellKnownMap(Type memberType, FallbackStrategy emptyValueStrategy,
        out ICellValueMapper mapper, out IFallbackItem emptyFallback, out IFallbackItem invalidFallback)
    {
        ...
        IFallbackItem ReconcileFallback(FallbackStrategy strategyToPursue, bool isEmpty)
        {
            ...
            if (isEmpty && isNullable) return new FixedValueFallback(null);

            if (strategyToPursue == FallbackStrategy.SetToDefaultValue ||
                emptyValueStrategy == FallbackStrategy.SetToDefaultValue)
        ...
        if (type == typeof(DateTime))
        {
            ...
            emptyFallback = ReconcileFallback(FallbackStrategy.ThrowIfPrimitive, true);
            invalidFallback = ReconcileFallback(FallbackStrategy.ThrowIfPrimitive, false);
        }
        ...
        else if (type == typeof(string) || type == typeof(object) || type == typeof(IConvertible))
        {
            ...
            emptyFallback = ReconcileFallback(FallbackStrategy.SetToDefaultValue, true);
            invalidFallback = ReconcileFallback(FallbackStrategy.SetToDefaultValue, false);
        }
    ...
    private static bool TryCreateEnumerableMap(MemberInfo member, FallbackStrategy emptyValueStrategy, out IMap map)
    {
        ...
        var method = TryCreateGenericEnumerableMapMethod.MakeGenericMethod(elementType);
    ...
    internal static bool TryCreateGenericEnumerableMap<TElement>(MemberInfo member,
        FallbackStrategy emptyValueStrategy, out ManyToOneEnumerableMap<TElement> map)
    {
        ...
        if (!TryCreatePrimitivePipeline(emptyValueStrategy, out ValuePipeline<TElement> elementMapping))
        {
            map = null;
            return false;
        }
        ...
        if (!TryGetCreateElementsFactory(member.MemberType(),
                out CreateElementsFactory<TElement> factory))
        {
            map = null;
            return false;
        }
    ...
    private static bool TryGetCreateElementsFactory<T>(Type memberType, out CreateElementsFactory<T> result)
    {
        ...
        if (memberType.IsImmutableEnumerableType())
        {
            var createRangeMethod = memberType.GetImmutableEnumerableCreateRangeMethod(typeof(T));
            result = elements => { return (IEnumerable<T>) createRangeMethod.Invoke(null, new object[] {elements}); };
            return true;
        }
        ...
        else if (memberType.ImplementsInterface(typeof(ICollection<T>)))
        {
            result = elements =>
            {
                var value = (ICollection<T>) Activator.CreateInstance(memberType);
                foreach (var element in elements) value.Add(element);
        ...
        if (ctor != null)
        {
            result = element => { return (IEnumerable<T>) Activator.CreateInstance(memberType, element); };
            return true;
        }

        // Check if the type has Add(T) such as BlockingCollection.
        var addMethod = memberType.GetMethod("Add", new[] {typeof(T)});
        if (addMethod != null)
        {
            result = elements =>
            {
                var value = (IEnumerable<T>) Activator.CreateInstance(memberType);
                foreach (var element in elements) addMethod.Invoke(value, new object[] {element});
    ...
    private static bool TryCreateDictionaryMap<T>(FallbackStrategy emptyValueStrategy, out IMap map)
    {
        ...
        if (!typeof(T).ImplementsGenericInterface(typeof(IEnumerable<>), out var keyValuePairType))
        {
            map = null;
            return false;
        }

        if (!keyValuePairType.IsGenericType ||
        ...
        var arguments = keyValuePairType.GenericTypeArguments;
        var keyType = arguments[0];
        var valueType = arguments[1];
        var method = TryCreateGenericDictionaryMapMethod.MakeGenericMethod(keyType, valueType);
    ...
    internal static bool TryCreateGenericDictionaryMap<TKey, TValue>(Type memberType,
        FallbackStrategy emptyValueStrategy, out ManyToOneDictionaryMap<TKey, TValue> map)
    {
        ...
        if (!TryCreatePrimitivePipeline(emptyValueStrategy, out ValuePipeline<TKey> keyPipeline))
        {
            map = null;
            return false;
        }

        if (!TryGetCreateDictionaryFactory(memberType,
                out CreateDictionaryFactory<TKey, TValue> factory))
        {
            map = null;
            return false;
        }
        ...
        map = new ManyToOneDictionaryMap<TKey, TValue>(defaultReader, keyPipeline, valuePipeline, factory);
        return true;
    }

    private static bool TryGetCreateDictionaryFactory<TKey, TValue>(Type memberType,
        out CreateDictionaryFactory<TKey, TValue> result)
    {
        if (memberType.IsImmutableDictionaryType())
        {
            var createRangeMethod = memberType.GetImmutableDictionaryCreateRangeMethod(typeof(TValue));
            result = elements =>
            {
                return (IDictionary<TKey, TValue>) createRangeMethod.Invoke(null, new object[] {elements});
            };
            return true;
        }
        ...
        if (memberType.GetTypeInfo().IsInterface)
        {
            if (memberType.GetTypeInfo().IsAssignableFrom(typeof(Dictionary<TKey, TValue>).GetTypeInfo()))
            {
                result = elements =>
                {
                    ...
                    foreach (var keyValuePair in elements) dictionary.Add(keyValuePair.Key, keyValuePair.Value);
        ...
        else if (memberType.ImplementsInterface(typeof(IDictionary<TKey, TValue>)))
        {
            ...
            {
                var dictionary =
                    (IDictionary<TKey, TValue>) Activator.CreateInstance(memberType);
                foreach (var keyValuePair in elements) dictionary.Add(keyValuePair);
    ...
    public static bool TryCreateClassMap<T>(FallbackStrategy emptyValueStrategy, out ExcelClassMap<T> classMap)
    {
        ...
        IEnumerable<MemberInfo> properties = type.GetRuntimeProperties()
            .Where(p => p.CanWrite && p.SetMethod.IsPublic && !p.SetMethod.IsStatic);
        ...
        foreach (var member in properties.Concat(fields))
        {
            ...
            if (Attribute.IsDefined(member, typeof(ExcelIgnoreAttribute))) continue;

            // Infer the mapping for each member (property/field) belonging to the type.
            var memberType = member.MemberType();
            var method = TryCreateMemberMapMethod.MakeGenericMethod(memberType);
            if (memberType == type)
                throw new ExcelMappingException(
                    $"Cannot map recursive property \"{member.Name}\" of type {memberType}. Consider applying the ExcelIgnore attribute.");
    ...
    internal static bool TryAutoMap<T>(FallbackStrategy emptyValueStrategy, out IMap result)
    {
        ...
        if (TryCreateClassMap(emptyValueStrategy, out ExcelClassMap<T> classMap))
        {
            result = classMap;
            return true;
        }
