...
public class ChangeTypeMapper : ICellValueMapper
{
    ...
    public ChangeTypeMapper(Type type)
    {
        ...
        if (!type.ImplementsInterface(typeof(IConvertible)))
            throw new ArgumentException(
                $"Type \"{type}\" must implement IConvertible to support Convert.ChangeType.", nameof(type));
    ...
    public CellValueMapperResult MapCellValue(ReadCellValueResult readResult)
    {
        ...
        {
            object result;
            if (Type.Name == "UInt32" && readResult.StringValue == "-1")
                result = Convert.ChangeType(uint.MaxValue.ToString(), Type);
            else if (Type.Name == "UInt64" && readResult.StringValue == "-1")
                result = Convert.ChangeType(ulong.MaxValue.ToString(), Type);
