...
public class ExcelSheet
{
        ...
        set
        {
            if (Heading != null)
                throw new InvalidOperationException(
                    "The heading has already been read. Set this property before reading any rows.");
        ...
        set
        {
            if (value < 0)
                throw new ArgumentOutOfRangeException(nameof(value), value,
                    "The index of the heading must be positive or zero.");

            if (!HasHeading) throw new InvalidOperationException("The sheet has no heading.");

            if (Heading != null) throw new InvalidOperationException("The heading has already been read.");
    ...
    {
        if (!HasHeading) throw new ExcelMappingException($"Sheet \"{Name}\" has no heading.");

        if (Heading != null) throw new ExcelMappingException($"Already read heading in sheet \"{Name}\".");
    ...
    {
        if (HasHeading && Heading == null) ReadHeading();

        while (TryReadRow(out T row)) yield return row;
    }
    ...
    ///     The zero-based index from the first row of the document (including the header) of the range of
    ///     rows to map from.
    /// </param>
    ...
    public IEnumerable<T> ReadRows<T>(int startIndex, int count)
    {
        ...
        for (var i = 0; i < count; i++) yield return ReadRow<T>();
    }
    ...
    public T ReadRow<T>()
    {
        if (!TryReadRow(out T value)) throw new ExcelMappingException($"No more rows in \"{Name}\".");
        ...
        if (!Importer.Configuration.TryGetClassMap<T>(out var classMap))
        {
            if (!HasHeading)
                throw new ExcelMappingException(
                    $"Cannot auto-map type \"{typeof(T)}\" as the sheet has no heading.");

            if (!AutoMapper.TryAutoMap<T>(FallbackStrategy.ThrowIfPrimitive, out var map))
