...
public class ExcelImporter : IDisposable
{
    ...
    /// </summary>
    public ExcelImporterConfiguration Configuration { get; } = new();
    ...
    {
        while (TryReadSheet(out var sheet)) yield return sheet;
    ...
    {
        if (!TryReadSheet(out var sheet)) throw new ExcelMappingException("No more sheets.");
    ...
    public ExcelSheet ReadSheet(int sheetIndex)
    {
        if (!TryReadSheet(sheetIndex, out var sheet))
            throw new ArgumentOutOfRangeException(nameof(sheetIndex), sheetIndex,
                $"The sheet index {SheetIndex} must be between 0 and {NumberOfSheets}.");
    ...
    public bool TryReadSheet(string sheetName, out ExcelSheet sheet)
    {
        ...
        while (TryReadSheet(out var currentSheet))
    ...
    public bool TryReadSheet(int sheetIndex, out ExcelSheet sheet)
    {
        ...
        if (sheetIndex < 0 || sheetIndex > NumberOfSheets - 1) return false;

        ResetReader();
        for (var i = 0; i < sheetIndex; i++) Reader.NextResult();
    ...
    internal void MoveToSheet(ExcelSheet sheet)
    {
        ...
        if (SheetIndex == sheet.Index) return;

        // Read up to the current sheet.
        Reader.Reset();
        for (var i = 0; i < sheet.Index; i++) Reader.NextResult();

        // If the header has already been read, skip past it.
        if (sheet.HasHeading && sheet.Heading != null) sheet.ReadPastHeading();

        // Read up to the current row.
        for (var i = 0; i <= sheet.CurrentRowIndex; i++) Reader.Read();
