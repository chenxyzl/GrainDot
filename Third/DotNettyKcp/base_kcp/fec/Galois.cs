...
public sealed class Galois
{
    ...
    public static readonly sbyte[] EXP_TABLE =
    {
    ...
    public static byte divide(byte a, byte b)
    {
        ...
        if (b == 0) throw new Exception("Argument 'divisor' is 0");
    ...
    public static byte exp(byte a, int n)
    {
        ...
        while (255 <= logResult) logResult -= 255;
    ...
    {
        var result = new short[FIELD_SIZE];
        for (var i = 0; i < FIELD_SIZE; i++) result[i] = -1; // -1 means "not set"

        var b = 1;
        for (var log = 0; log < FIELD_SIZE - 1; log++)
        {
            if (result[b] != -1) throw new Exception("BUG: duplicate logarithm (bad polynomial?)");

            result[b] = (short) log;
            b = b << 1;
            if (FIELD_SIZE <= b) b = (b - FIELD_SIZE) ^ polynomial;
        }
    ...
    {
        var result = new byte [FIELD_SIZE * 2 - 2];
    ...
    {
        var result = new byte[EXP_TABLE.Length];
        for (var i = 0; i < EXP_TABLE.Length; i++) result[i] = (byte) EXP_TABLE[i];
        return result;
    }
    ...
    public static byte[][] generateMultiplicationTable()
    {
        ...
        for (var a = 0; a < FIELD_SIZE; a++)
        {
            result[a] = new byte[256];
            for (var b = 0; b < FIELD_SIZE; b++) result[a][b] = multiply((byte) a, (byte) b);
        }
