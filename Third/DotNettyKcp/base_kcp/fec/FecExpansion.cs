...
{
    private static readonly InputOutputByteBufTableCodingLoop LOOP = new();
    ...
    public static void encodeParity(this ReedSolomon reedSolomon, IByteBuffer[] shards, int offset, int byteCount)
    {
        ...
        var outputs = new IByteBuffer [reedSolomon.ParityShardCount];
...
//        System.arraycopy(shards, dataShardCount, outputs, 0, parityShardCount);
        ...
        int byteCount)
    {
        ...
        var shardLength = shards[0].ReadableBytes;
        ...
        int byteCount)
    {
        ...
        // nothing to do.
        var numberPresent = 0;
        for (var i = 0; i < reedSolomon.TotalShardCount; i++)
            if (shardPresent[i])
                numberPresent += 1;
        ...
        if (numberPresent < reedSolomon.DataShardCount) throw new Exception("Not enough shards present");
        ...
        var subMatrix = new Matrix(reedSolomon.DataShardCount, reedSolomon.DataShardCount);
        var subShards = new IByteBuffer[reedSolomon.DataShardCount];
        {
            var subMatrixRow = 0;
            for (var matrixRow = 0;
                ...
                {
                    for (var c = 0; c < reedSolomon.DataShardCount; c++)
        ...
        var outputs = new IByteBuffer[reedSolomon.ParityShardCount];
        var matrixRows = new byte [reedSolomon.ParityShardCount][];
        var outputCount = 0;
        for (var iShard = 0; iShard < reedSolomon.DataShardCount; iShard++)
        ...
        outputCount = 0;
        for (var iShard = reedSolomon.DataShardCount; iShard < reedSolomon.TotalShardCount; iShard++)
            if (!shardPresent[iShard])
            {
                outputs[outputCount] = shards[iShard];
