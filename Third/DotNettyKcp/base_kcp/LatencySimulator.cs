...
public class LatencySimulator : KcpOutput
{
    ...
    private readonly LinkedList<DelayPacket> p12 = new();
    private readonly LinkedList<DelayPacket> p21 = new();
    private readonly Random r12 = new();
    private readonly Random r21 = new();
    ...
    public int send(int peer, IByteBuffer data)
    {
        ...
        if (rnd < lostrate) return 0;

        var pkt = new DelayPacket();
        pkt.init(data);
        current = DateTime.Now.Ticks / 10000;
        var delay = rttmin;
        if (rttmax > rttmin) delay += _random.Next(10000) % (rttmax - rttmin);
    ...
    public int recv(int peer, IByteBuffer data)
    {
        ...
        if (peer == 0)
        {
            if (p21.Count == 0) return -1;

            pkt = p21.First.Value;
        }
        else
        {
            if (p12.Count == 0) return -1;
        ...
        var maxsize = pkt.getPtr().ReadableBytes;
...
//            latencySimulator.BenchmarkFlush();
    ...
    {
        var kcp = new Kcp(1, new LatencySimulator());
        for (var i = 0; i < 1000; i++)
        {
            var segment = Segment.createSegment(null);
        ...
        //预热
        for (var i = 0; i < 1000000; i++) kcp.flush(false, kcp.currentMs());

        var start = kcp.currentMs();
        for (var i = 0; i < 200000; i++) kcp.flush(false, kcp.currentMs());
    ...
    {
        var vnet = new LatencySimulator();
        vnet.init(20, 600, 600);
        var output1 = new TestOutPut(vnet, 0);
        var output2 = new TestOutPut(vnet, 1);


        var kcp1 = new Kcp(0x11223344, output1);
        var kcp2 = new Kcp(0x11223344, output2);
        ...
        for (;;)
        {
            ...
            for (;;)
            {
                var buffer = PooledByteBufferAllocator.Default.DirectBuffer(2000);
//                    Console.WriteLine("buffer:" +buffer.AddressOfPinnedMemory().ToString());
            ...
            for (;;)
            {
                var buffer = PooledByteBufferAllocator.Default.Buffer(2000);
                try
                {
            ...
            // kcp2接收到任何包都返回回去
            var bufList = new List<IByteBuffer>();
            kcp2.recv(bufList);
        ...
        Console.WriteLine("snmp: " + Snmp.snmp);
    }


    private class TestOutPut : KcpOutput
    {
        private readonly int id;
        private readonly LatencySimulator vnet;
