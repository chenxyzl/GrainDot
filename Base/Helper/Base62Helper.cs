...
public static class Base62Helper
{
    ...
    public static string EncodeBytes(byte[] bytes)
    {
        ...
        while (offset < bytes.Length)
        {
            ...
            {
                return EncodeUInt64(BigEndianToUInt64(bytes, offset, length));
            }
    ...
    public static byte[] DecodeBytes(string base62)
    {
        ...
        while (offset < contentLength)
        {
            ...
            {
                return UInt64ToBigEndian(DecodeUInt64(base62.Substring(offset, length)), blockSize);
            }
    ...
    /// <param name="length">
    ///     The number of bytes to read. Note that although
    ///     bytes are in big endian order, it is the least significant portion of
    ///     the UInt64 that is read when length is less than 8.
    /// </param>
    ...
    /// <param name="length">
    ///     The number of bytes to encode. Note that although
    ///     bytes are in big endian order, it is the least significant portion of
    ///     the UInt64 that is encoded when length is less than 8.
    /// </param>
