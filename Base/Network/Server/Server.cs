...
namespace Base.Network.Server
{
    ...
    public class Server<T> where T : BaseActor
    {
        ...
        /// <param name="reader">The buffer reader from received message.</param>
        public delegate void ServerPacketHandler(IClient client, IBufferReader reader);
        ...
        /// <summary>
        /// Creates a new instance of a <see cref="Server"/>.
        /// </summary>
        ...
        public Server(ushort port = 7171, ushort maxMessageBuffer = 4096, ushort maxClientConnections = 1000)
        {
            try
            {
                _port = port;
        ...
        /// </summary>
        /// <param name="client">The connected client.</param>
        /// <param name="buffer">The buffer received from message.</param>
        private void OnMessageReceived(IClient client, IBufferReader reader)
        {
            try
            {
        ...
        /// <param name="buffer">The buffer of received message.</param>
        public void SendMessage(IClient client, BufferWriter buffer)
        {
            try
            {
        ...
        /// </summary>
        /// <param name="buffer">The buffer of received message.</param>
        public void SendMessageToAll(BufferWriter buffer)
        {
            try
            {
        ...
        /// <param name="client">The client instance.</param>
        /// <param name="buffer">The buffer of received message.</param>
        public void SendMessageToAllMinus(IClient client, BufferWriter buffer)
        {
            try
            {
                foreach (var clientToSend in GetAllClients().Where(c => c.IsConnected && c.Id != client.Id))
                    SendMessage(clientToSend, buffer);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error: {ex.Message}.");
