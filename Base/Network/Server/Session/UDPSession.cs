...
namespace Base.Network.Server
{
    ...
    public class UDPSession : Session
    {
        ...
        /// <summary>
        /// Creates a new instance of a <see cref="UDPSession"/>.
        /// </summary>
        ...
        public UDPSession(ushort id, UdpNetworkListener socketClient, EndPoint remoteEndPoint, MessageReceivedHandler messageReceivedHandler, ClientDisconnectedHandler clientDisconnectedHandler, ushort maxMessageBuffer)
        {
            try
            {
        ...
        public void ReceiveDataCallback(byte[] data)
        {
            try
            {
                _messageReceivedHandler?.Invoke(this, BufferReader.Create(data, 0, data.Length));
            }
            catch (Exception)
            {
                _clientDisconnectedHandler(this);
